import type { WorkspaceScriptsPath } from "../schemas";
import { getTags } from "../utils/internal/get-tags";
...
export const WORKSPACE_SCRIPT_FILES = {
	...
	// = Boom Commands  =
	// ==================
	[`./${BASE_DIR}/boom/index.ts`]: /* ts */ `${"#!/usr/bin/env bun"}
...
console.log("Cleanup complete.");`,
	...
	[`./${BASE_DIR}/boom/refresh.ts`]: /* ts */ `${"#!/usr/bin/env bun"}
...
await Bun.spawn(["bun", "install"]);`,
	...
	[`./${BASE_DIR}/build/index.ts`]: /* ts */ `${"#!/usr/bin/env bun"}
await Bun.spawn(["bunx", "nx", "run-many", "--target=build", "--projects=tag:${getTags()}"]);`,

	// ===================
	// = Commit Commands =
	// ===================
	[`./${BASE_DIR}/commit/index.ts`]: /* ts */ `${"#!/usr/bin/env bun"}
await Bun.spawn(["bunx", "cz"]);`,
	[`./${BASE_DIR}/commit/protect.ts`]: /* ts */ `${"#!/usr/bin/env bun"}
...
console.log(\`Committing to \${currentBranch}\`);`,
	...
	// = Docs Command  =
	// =================
	[`./${BASE_DIR}/docs/index.ts`]: /* ts */ `${"#!/usr/bin/env bun"}
await Bun.spawn(["bunx", "typedoc"]);`,

	// =====================
	// = Generate Command  =
	// =====================
	[`./${BASE_DIR}/generate/index.ts`]: /* ts */ `${"#!/usr/bin/env bun"}
await Bun.spawn(["bunx", "nx", "workspace-generator"]);`,

	[`./${BASE_DIR}/generate/refresh.ts`]: /* ts */ `${"#!/usr/bin/env bun"}
...
`,
	...
	// = Lint Commands  =
	// ==================
	[`./${BASE_DIR}/lint/index.ts`]: `await Bun.spawn(["bunx", "nx", "run-many", "--target=biome-lint", "--write=true", "--projects=tag:${getTags()}"]);`,

	// ==========================
	// = Local Registry Commands =
	// ==========================
	[`./${BASE_DIR}/local-registry/start.ts`]: /* ts */ `${"#!/usr/bin/env bun"}
await Bun.spawn(["bunx", "verdaccio"]);`,

	[`./${BASE_DIR}/local-registry/publish.ts`]: /* ts */ `${"#!/usr/bin/env bun"}
...
await Bun.spawn(["npm", "config", "set", "registry", "https://registry.npmjs.org"]);`,
	...
	[`./${BASE_DIR}/local-registry/stop.ts`]: /* ts */ `${"#!/usr/bin/env bun"}
...
await Bun.spawn(["bunx", "pm2", "delete", "verdaccio"]);`,
	...
	// = Release Commands =
	// ====================
	[`./${BASE_DIR}/release/index.ts`]: /* ts */ `${"#!/usr/bin/env bun"}
await Bun.spawn([LEFTHOOK = "0", "bunx", "nx", "release", "-y"]);`,

	[`./${BASE_DIR}/release/dry-run.ts`]: /* ts */ `${"#!/usr/bin/env bun"}
await Bun.spawn([LEFTHOOK = "0", "bunx", "nx", "release", "--dry-run", "--skip-publish"]);`,

	// ==================
	// = Sort Commands  =
	// ==================
	[`./${BASE_DIR}/sort/index.ts`]: /* ts */ `${"#!/usr/bin/env bun"}
await Bun.spawn(["bunx", "nx", "run-many", "--target=sort", "--projects=tag:${getTags()}"]);`,
} satisfies Record<WorkspaceScriptsPath, string>;
